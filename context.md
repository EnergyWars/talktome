...
| Gemini-API-Schlüssel-Verwaltung (Eingabefeld nur ohne hinterlegten Schlüssel) | Fertig | `data/security/SecureApiKeyStore.kt`, `data/gemini/*`, `ui/gemini/*` |
| Gemini-Modellauswahl (Pro/Flash/Flash-Lite, persistiert, wirkt auf alle Gemini-Aufrufe) | Fertig | `data/gemini/GeminiModel.kt`, `data/settings/SettingsRepository.kt`, `ui/gemini/*` |
...
- **Gemini-Modellauswahl**: `GeminiModel`-Enum (`PRO`/`FLASH`/`FLASH_LITE` mit Google-`wireId`, Default `FLASH`) wird über `SettingsRepository.geminiModel` (unverschlüsseltes DataStore, wie `themeMode`) persistiert. Jede Aufrufstelle von `GeminiClient.generateContent` (`OutgoingMessageRepository`, `InboxRepository`, `VentingViewModel`) liest das aktuell gewählte Modell unmittelbar vor dem Aufruf und übergibt dessen `wireId` explizit als `model`-Parameter, statt sich auf den clientseitigen Default zu verlassen.
- **API-Schlüssel-UI-Sichtbarkeit**: `GeminiSettingsScreen` zeigt das Eingabefeld/„Speichern & testen“ nur, wenn kein Schlüssel hinterlegt ist (`hasApiKey == false`); ist ein Schlüssel gespeichert, wird ausschließlich der „Schlüssel hinterlegt“-Hinweis plus „Schlüssel entfernen“ angezeigt. Ein neuer Schlüssel kann erst nach dem Entfernen eingetragen werden. Die Modellauswahl ist davon unabhängig immer sichtbar.
